...
#include "ns3/log.h"
#include "do-tcp-rx-buffer.h"

NS_LOG_COMPONENT_DEFINE ("DoTcpRxBuffer");

namespace ns3 {

TypeId
DoTcpRxBuffer::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::DoTcpRxBuffer")
    .SetParent<Object> ()
    .AddConstructor<DoTcpRxBuffer> ()
    .AddTraceSource ("NextRxSequence",
                     "Next sequence number expected (RCV.NXT)",
                     MakeTraceSourceAccessor (&DoTcpRxBuffer::m_nextRxSeq))
  ;
  return tid;
}
 ...
 * RcvBufSize is passed to DoTcpRxBuffer by DoTcpSocketBase::SetRcvBufSize() and in
 * turn, DoTcpRxBuffer:SetMaxBufferSize(). Therefore, the m_maxBuffer value
 * initialized below is insignificant.
 */
DoTcpRxBuffer::DoTcpRxBuffer (uint32_t n)
  : m_nextRxSeq (n), m_gotFin (false), m_size (0), m_maxBuffer (32768), m_availBytes (0)
{
}

DoTcpRxBuffer::~DoTcpRxBuffer ()
{
}

SequenceNumber32
DoTcpRxBuffer::NextRxSequence (void) const
{
  return m_nextRxSeq;
}

void
DoTcpRxBuffer::SetNextRxSequence (const SequenceNumber32& s)
{
  m_nextRxSeq = s;
}

uint32_t
DoTcpRxBuffer::MaxBufferSize (void) const
{
  return m_maxBuffer;
}

void
DoTcpRxBuffer::SetMaxBufferSize (uint32_t s)
{
  m_maxBuffer = s;
}

uint32_t
DoTcpRxBuffer::Size (void) const
{
  return m_size;
}

uint32_t
DoTcpRxBuffer::Available () const
{
  return m_availBytes;
}

void
DoTcpRxBuffer::IncNextRxSequence ()
{
...
// Return the lowest sequence number that this DoTcpRxBuffer cannot accept
SequenceNumber32
DoTcpRxBuffer::MaxRxSequence (void) const
{
  if (m_gotFin)
...
void
DoTcpRxBuffer::SetFinSequence (const SequenceNumber32& s)
{
  NS_LOG_FUNCTION (this);
...
bool
DoTcpRxBuffer::Finished (void)
{
  return (m_gotFin && m_finSeq < m_nextRxSeq);
}

bool
DoTcpRxBuffer::Add (Ptr<Packet> p, DoTcpHeader const& tcph)
{
...
Ptr<Packet>
DoTcpRxBuffer::Extract (uint32_t maxSize)
{
  NS_LOG_FUNCTION (this << maxSize);

  uint32_t extractSize = std::min (maxSize, m_availBytes);
  NS_LOG_LOGIC ("Requested to extract " << extractSize << " bytes from DoTcpRxBuffer of size=" << m_size);
