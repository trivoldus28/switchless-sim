...
#include "p2p-2d-torus.h"
 
NS_LOG_COMPONENT_DEFINE ("PointToPoint2DTorusHelper");

namespace ns3 {

PointToPoint2DTorusHelper::PointToPoint2DTorusHelper (uint32_t nRows, 
                                                uint32_t nCols, 
...
PointToPoint2DTorusHelper::~PointToPoint2DTorusHelper ()
{
}

void
PointToPoint2DTorusHelper::InstallStack (InternetStackHelper stack)
{
...
void
PointToPoint2DTorusHelper::AssignIpv4Addresses (Ipv4AddressHelper rowIp, Ipv4AddressHelper colIp)
{
...
void
PointToPoint2DTorusHelper::AssignIpv6Addresses(Ipv6Address addrBase, Ipv6Prefix prefix)
{
...
void
PointToPoint2DTorusHelper::BoundingBox (double ulx, double uly,
...
Ptr<Node> 
PointToPoint2DTorusHelper::GetNode (uint32_t row, uint32_t col)
{
  if (row > m_nodes.size () - 1 || 
      col > m_nodes.at (row).GetN () - 1) 
    {
      NS_FATAL_ERROR ("Index out of bounds in PointToPoint2DTorusHelper::GetNode.");
    }
...
Ipv4Address
PointToPoint2DTorusHelper::GetIpv4Address (uint32_t row, uint32_t col)
{
  if (row > m_nodes.size () - 1 || 
      col > m_nodes.at (row).GetN () - 1) 
    {
      NS_FATAL_ERROR ("Index out of bounds in PointToPoint2DTorusHelper::GetIpv4Address.");
    }
...
Ipv6Address
PointToPoint2DTorusHelper::GetIpv6Address (uint32_t row, uint32_t col)
{
  if (row > m_nodes.size () - 1 ||
      col > m_nodes.at (row).GetN () - 1)
    {
