...
main (int argc, char * argv[])
{
    ...
    topologytype = atoi(getCmdOption(argv,argv+argc,"-topology")); // 1 or 2 or 3
    topo_sub1 = atoi(getCmdOption(argv,argv+argc, "-t1")); // leaf-fan-out or row or m
    topo_sub2 = atoi(getCmdOption(argv,argv+argc, "-t2")); // non-leaf-fan-out or column or n
    nNodes = atoi(getCmdOption(argv,argv+argc,"-nodecount"));
    nIterations = atoi(getCmdOption(argv,argv+argc,"-iter"));
    nPacketSize = atoi(getCmdOption(argv,argv+argc, "-packetsize"));
    nSender = atoi(getCmdOption(argv,argv+argc, "-sendercount"));
    nReceiver = atoi(getCmdOption(argv,argv+argc, "-receivercount)"));
    intervaltype = atoi(getCmdOption(argv,argv+argc, "-intervaltype)")); // 1 or 2
    nintervalsize = atoi(getCmdOption(argv,argv+argc, "-intervalsize)")); // only for fixed
    nMininterval = atoi(getCmdOption(argv,argv+argc, "-intervalmin)"));
    nMaxinterval = atoi(getCmdOption(argv,argv+argc, "-intervalmax)"));
    synchronized = atoi(getCmdOption(argv,argv+argc, "-sync")); // 0 or 1
    // Format Conversion
    ...
    for (auto it = senderSet.begin(); it != senderSet.end(); it++){
